    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $result = match ($route) {
                ...
                '/api/material-save' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->saveMaterial($payload)),
                '/api/material-delete' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->deleteMaterial((int) ($payload['id'] ?? $payload['material_id'] ?? 0), $payload)),
                '/api/material-restore' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->restoreMaterial((int) ($payload['id'] ?? $payload['material_id'] ?? 0), $payload)),
                '/api/inventory-stock-save' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->saveBranchStock($payload)),
                '/api/recipe-save' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->saveRecipe($payload)),
                '/api/recipe-delete' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->deleteRecipe((int) ($payload['id'] ?? $payload['recipe_id'] ?? 0), $payload)),
                '/api/recipe-restore' => $this->withEndpoint($route, $auth, $payload, fn () => (new Inventory())->restoreRecipe((int) ($payload['id'] ?? $payload['recipe_id'] ?? 0), $payload)),
    ...
    private function route(): string
    {
        ...
        if (isset($_GET['action'])) {
            return match ((string) $_GET['action']) {
                ...
                'material_save' => '/api/material-save',
                'material_delete' => '/api/material-delete',
                'material_restore' => '/api/material-restore',
                'inventory_stock_save' => '/api/inventory-stock-save',
                'recipe_save' => '/api/recipe-save',
